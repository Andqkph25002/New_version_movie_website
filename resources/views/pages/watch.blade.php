                    ...
                    <div class="col-xs-6">
                        <div class="yoast_breadcrumb hidden-xs"><span><span><a href="">{{ $movie->title }}</a> »
                                    <span><a
                                        ...
                                        » <span class="breadcrumb_last" aria-current="page"></span></span></span></span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div id="halim-list-server">
                        <ul class="nav nav-tabs" role="tablist">
                            ...
                            @elseif($movie->resolution == 4)
                                <li role="presentation" class="active server-1"><a href="#server-0"
                                        aria-controls="server-0" role="tab" data-toggle="tab"><i
                                            class="hl-server"></i> FULL HD</a></li>
                            @else
                                <li role="presentation" class="active server-1"><a href="#server-0"
                                        aria-controls="server-0" role="tab" data-toggle="tab"><i
                            ...
                            <div role="tabpanel" class="tab-pane active server-1" id="server-0">
                                <div class="halim-server">
                                    <ul class="halim-list-eps">
                                        @foreach ($movie->rEpisode as $key => $item)
                                            <a
                                                href="{{ route('watch.page', ['slug' => $movie->slug, 'tap' => $item->episode]) }}">
                                                <li class="halim-episode"><span
                                                        class="halim-btn halim-btn-2 {{ $tapphim == $item->episode ? 'active' : '' }} halim-info-1-1 box-shadow"
                                                        data-post-id="37976" data-server="1"
                                                        data-episode="{{ $item->episode }}" data-position="first"
                                                        data-embed="0"
                                                        data-title="Xem phim {{ $movie->title }} - Tập {{ $item->episode }} - {{ $movie->name_eng }} - vietsub + Thuyết Minh"
                                                        data-h1="{{ $movie->title }} - tập {{ $item->episode }}">{{ $item->episode }}</span>
                                                </li>
                                            </a>
                                        @endforeach
